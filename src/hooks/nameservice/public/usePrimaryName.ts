...
export const getPrimaryNameQueryFn =
  ...
  async <TParams extends UsePrimaryNameParameters>({
    ...
    if (res.match) {
      ...
      // Some names like "MetaMask.etc" may resolve correctly but are not normalized
      // (should be "metamask.etc"). We fetch the raw name directly from the Universal Resolver
